...
experiment = 'zurich_garage'  # final_challenge | zurich_garage
all_detection_df = pd.read_csv('./' + experiment + '.csv')
print(all_detection_df.head())

#%% Process and cluster for each class
# data csv
# person | stop | backpack | umbrella | bottle | clock |
class_label_list = ['person', 'stop',
                    'backpack', 'umbrella',
                    'bottle', 'clock',
                    'car']
test_idxs = [0, 1, 5, 6]
for idx in test_idxs:
    ...
    instances_df = all_detection_df[all_detection_df["Class"] == class_label]
