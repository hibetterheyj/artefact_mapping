    ...
    def save_to_csv(self):
        ...
        while not rospy.is_shutdown():
            if (self.idx <= current_idx):
                ...
                if self.artefact_class == 'person':
                    person_list.append(new_artefact)
                    person_pd = pd.DataFrame(
                        columns=name, data=person_list)
                    person_pd.to_csv(self.bag_name + "_person.csv",
                                   encoding='utf-8', index=False)
                elif self.artefact_class == 'stop':
                    stopsign_list.append(new_artefact)
                    stopsign_pd = pd.DataFrame(
                        columns=name, data=stopsign_list)
                    stopsign_pd.to_csv(self.bag_name + "_stopsign.csv",
                                     encoding='utf-8', index=False)
                elif self.artefact_class == 'backpack':
                    backpack_list.append(new_artefact)
                    backpack_pd = pd.DataFrame(
                        columns=name, data=backpack_list)
                    backpack_pd.to_csv(self.bag_name + "_backpack.csv",
                                       encoding='utf-8', index=False)
                elif self.artefact_class == 'umbrella':
                    umbrella_list.append(new_artefact)
                    umbrella_pd = pd.DataFrame(
                        columns=name, data=umbrella_list)
                    umbrella_pd.to_csv(self.bag_name + "_umbrella.csv",
                                       encoding='utf-8', index=False)
                elif self.artefact_class == 'bottle':
                    bottle_list.append(new_artefact)
                    bottle_pd = pd.DataFrame(
                        columns=name, data=bottle_list)
                    bottle_pd.to_csv(self.bag_name + "_bottle.csv",
                                       encoding='utf-8', index=False)
                elif self.artefact_class == 'clock':
                    clock_list.append(new_artefact)
                    clock_pd = pd.DataFrame(
                        columns=name, data=clock_list)
                    clock_pd.to_csv(self.bag_name + "_clock.csv",
                                       encoding='utf-8', index=False)
